...
const Taskbar: React.FC<TaskbarProps> = React.memo(({ os, messenger, time }) => {
  ...
  if (isMac) {
    return (
      <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 bg-white/20 backdrop-blur-xl border border-white/20 rounded-2xl h-16 px-4 flex items-center gap-4 shadow-2xl z-30">
  ...
  // Windows Taskbar
  return (
    <div className="absolute bottom-0 w-full h-10 bg-[#f3f3f3]/95 backdrop-blur border-t border-white/50 flex items-center justify-between px-2 z-30">
